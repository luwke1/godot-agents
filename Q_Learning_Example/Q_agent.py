...
class Q_agent:
    def __init__(
        ...
        self.action_size = action_space_size            # the number of moves in the action space
    ...
    def get_action(self, obs) -> int:
        if obs not in self.q_values.keys():
            self.q_values[obs] = [0.0] * self.action_size
    ...
    def update(self, obs, action, reward, terminated, next_obs):
        ...
        if next_obs not in self.q_values.keys():
            self.q_values[next_obs] = [0.0] * self.action_size
